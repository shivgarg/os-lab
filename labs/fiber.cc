...
void g(addr_t* pmain_stack, addr_t* pf_stack, char* pret, bool* pdone){
   ...
   hoh_debug("in fiber g");
	...
	for(i=1;i<arg;i++)
		for(j=1;j<arg;j++)
			for(k=1;k<arg;k++)
				{ans=i*j/k;stack_saverestore(f_stack,main_stack);}
	ans=i*j/k;
	int po=0;
	int p=ans;
	while((p/10)!=0)
...
void shell_step_fiber(shellstate_t& shellstate, addr_t& main_stack, addr_t& f_stack, addr_t f_array, uint32_t f_arraysize){
 		...
 		if(shellstate.init_fiber)
 		{
 			...
 			stack_init4(f_stack,&f_array,f_arraysize,&g,main_stack,f_stack,shellstate.fiber_out,&shellstate.done);
 		}
